...
void Renderer::render(const std::vector<std::shared_ptr<Object3D>> scene, const Camera& camera)
{
    ...
    for (const auto& object : scene) {
        ...
        if (!object->hasGeometry()) {
            ...
            const auto projectedVertices = projectVertices(*mesh, camera);

            drawFilledTriangles(projectedVertices, mesh->getGeometry().indices, 0x00000033);
            drawWireframe(projectedVertices, mesh->getGeometry().indices);
